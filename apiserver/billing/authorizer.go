...
//go:generate go run github.com/golang/mock/mockgen -destination=./mock/$GOFILE -package mock_billing k8s.io/apiserver/pkg/authorization/authorizer Authorizer

// rbacAuthorizer processes authorization requests for `billingentities.billing.appuio.io` and checks them based on rbac rules for `billingentities.rbac.appuio.io`
...
func (a rbacAuthorizer) Authorize(ctx context.Context, attr authorizer.Attributes) error {
	if attr.GetResource() != "billingentities" {
